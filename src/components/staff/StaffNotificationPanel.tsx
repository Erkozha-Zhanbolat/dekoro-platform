...
import { formatNotificationRelativeTime } from "@/lib/staff/notifications";

const focusRing =
  "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0F766E] focus-visible:ring-offset-2";

export type StaffNotificationPanelProps = {
  panelId: string;
  ...
  onMarkAll: () => void;
  onClose: () => void;
  onItemClick: (event: MouseEvent<HTMLAnchorElement>, item: StaffNotification) => void;
};

export default function StaffNotificationPanel({
  panelId,
  items,
  loading,
  error,
          ...
          <ul className="divide-y divide-neutral-100">
            {items.map((item) => {
              ...
              return (
                <li key={item.id}>
                  <Link
                    href={item.action_url || "/staff/notifications"}
                    ...
                    <div className="flex gap-2">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="mt-1 text-[11px] text-neutral-400">
                          {formatNotificationRelativeTime(item.created_at, nowMs)}
                        </p>
                      </div>
                    </div>
                  </Link>
