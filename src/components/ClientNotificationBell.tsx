...
import {
  formatClientUnreadBadge,
  getClientUnreadNotificationCount,
  listClientNotificationsHeader,
...
export default function ClientNotificationBell({ profileId }: ClientNotificationBellProps) {
  ...
  // profileId changes remount via Header key={user.id} — no leftover inbox state.
  ...
  const refreshList = useCallback(async () => {
    ...
    setError(null);
    try {
      const rows = await listClientNotificationsHeader(HEADER_LIMIT);
      setItems(rows);
      setNowMs(Date.now());
    } catch (err) {
  ...
  useEffect(() => {
    const channel = supabase
      .channel(`client_notifications:${profileId}`)
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const badge = formatClientUnreadBadge(unread);
