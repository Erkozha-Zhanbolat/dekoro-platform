-- ============================================================
-- 027_data_lifecycle.sql
-- Stage 27 — Data Lifecycle Management (final audit hardened)
--
-- NOT applied by this change — apply by hand when ready.
-- Does NOT modify migrations 001–026 files.
--
-- Hardened vs draft:
--   - Director KPI excludes orders.is_test (CREATE OR REPLACE of 025 helpers/RPCs)
--   - Archives: compact manifest in Postgres + ZIP in private Storage (no full jsonb dump)
--   - Atomic test-order cleanup in one transaction (inventory restore + delete)
--   - Inventory restore idempotent (once per order / archive)
--   - Archive state machine: draft→ready→exported→db_cleaned→storage_cleanup_pending→cleaned
--   - Raw analytics cleanup requires aggregate coverage first
--
...
begin
  ...
  if to_regclass('public.analytics_events') is null then
    raise exception 'public.analytics_events missing — run 026 first.';
  end if;
  if to_regclass('public.analytics_sessions') is null then
    raise exception 'public.analytics_sessions missing — run 026 first.';
  end if;
  if to_regprocedure('public.staff_assert_active_admin()') is null then
    raise exception 'public.staff_assert_active_admin missing — run 024 first.';
  end if;
  if to_regprocedure('public.staff_release_order_reservations(uuid)') is null then
    raise exception 'public.staff_release_order_reservations missing — run 012 first.';
  end if;
  if to_regprocedure('public.admin_dashboard_orders_money()') is null then
    raise exception 'admin_dashboard_orders_money missing — run 025 first.';
  end if;
end
$$;
...
comment on column public.orders.is_test is
  'Test/sandbox order. Excluded from director production KPI. Cleanup only via admin_execute_test_order_cleanup.';
...
-- ============================================================
-- 2. Retention settings (+ lifecycle timestamps)
-- ============================================================
...
create table if not exists public.data_retention_settings (
  ...
  raw_analytics_days integer not null default 90
    check (raw_analytics_days in (30, 90, 180, 365)),
  snapshots_days integer not null default 365
    check (snapshots_days >= 30 and snapshots_days <= 3660),
  test_archives_days integer null
    check (test_archives_days is null or (test_archives_days >= 30 and test_archives_days <= 3660)),
  last_aggregated_at timestamptz null,
  last_aggregated_from date null,
  last_aggregated_to date null,
  last_cleanup_at timestamptz null,
  last_cleanup_cutoff timestamptz null,
...
alter table public.data_retention_settings
  add column if not exists last_aggregated_at timestamptz null;
alter table public.data_retention_settings
  add column if not exists last_aggregated_from date null;
alter table public.data_retention_settings
  add column if not exists last_aggregated_to date null;
alter table public.data_retention_settings
  add column if not exists last_cleanup_at timestamptz null;
alter table public.data_retention_settings
  add column if not exists last_cleanup_cutoff timestamptz null;

comment on table public.data_retention_settings is
  'Retention preferences + analytics lifecycle timestamps. Auto-cron NOT enabled.';

insert into public.data_retention_settings (singleton_key)
values ('default')
on conflict (singleton_key) do nothing;

alter table public.data_retention_settings enable row level security;
revoke all on table public.data_retention_settings from public, anon, authenticated;

-- ============================================================
-- 3. Permanent analytics aggregates
-- ============================================================

create table if not exists public.analytics_aggregates_daily (
...
create table if not exists public.analytics_aggregates_weekly (
...
create table if not exists public.analytics_aggregates_monthly (
...
alter table public.analytics_aggregates_weekly enable row level security;
alter table public.analytics_aggregates_monthly enable row level security;
revoke all on table public.analytics_aggregates_daily from public, anon, authenticated;
revoke all on table public.analytics_aggregates_weekly from public, anon, authenticated;
revoke all on table public.analytics_aggregates_monthly from public, anon, authenticated;

-- ============================================================
-- 4. data_archives — COMPACT ONLY (no full export jsonb)
-- Expected row size: ~2–15 KB (manifest), not megabytes.
-- Full ZIP lives in private Storage bucket data-archives.
-- ============================================================

create sequence if not exists public.data_archives_number_seq
...
create table if not exists public.data_archives (
  ...
  archive_number text not null unique
    default ('DEKORO-AR-' || lpad(nextval('public.data_archives_number_seq')::text, 6, '0')),
  archive_type text not null
    check (archive_type in ('weekly', 'monthly', 'test_orders', 'export', 'manual')),
  period_from date null,
  period_to date null,
  title text not null,
  -- State machine:
  -- draft → ready → exported → db_cleaned → storage_cleanup_pending → cleaned
  -- weekly/monthly stop at exported (reports do NOT delete production data)
  ...
  -- Compact manifest ONLY: row counts, financial/analytics totals,
  -- test-order fingerprints (ids + reservation snapshot), flags.
  -- NEVER store full orders/items/payments/events arrays here.
  manifest jsonb not null default '{}'::jsonb,
  checksum text null,
  export_file_path text null,
  export_bytes bigint null,
...
-- Migrate draft schema if an older 027 draft created payload/summary columns
do $$
begin
  if exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='payload'
  ) then
    alter table public.data_archives drop column if exists payload;
  end if;
  if exists (
    select 1 from information_schema.columns
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='archive_number'
  ) then
    alter table public.data_archives
      add column archive_number text;
    ...
    alter table public.data_archives
      add constraint data_archives_archive_number_key unique (archive_number);
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='manifest'
  ...
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='schema_version'
  ) then
    alter table public.data_archives add column schema_version integer not null default 1;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='checksum'
  ) then
    alter table public.data_archives add column checksum text null;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='export_file_path'
  ) then
    alter table public.data_archives add column export_file_path text null;
  end if;
  if exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='file_path'
  ) then
    update public.data_archives set export_file_path = coalesce(export_file_path, file_path);
    alter table public.data_archives drop column if exists file_path;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='export_bytes'
  ) then
    alter table public.data_archives add column export_bytes bigint null;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='exported_at'
  ) then
    alter table public.data_archives add column exported_at timestamptz null;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='db_cleaned_at'
  ) then
    alter table public.data_archives add column db_cleaned_at timestamptz null;
  end if;
  if not exists (
    select 1 from information_schema.columns
    where table_schema='public' and table_name='data_archives' and column_name='storage_cleaned_at'
  ) then
    alter table public.data_archives add column storage_cleaned_at timestamptz null;
  end if;

  -- Widen status check if an older 027 draft created a narrower CHECK.
  begin
  ...
  begin
    alter table public.data_archives
      add constraint data_archives_status_check check (status in (
        'draft', 'ready', 'exported', 'db_cleaned',
        'storage_cleanup_pending', 'cleaned', 'failed', 'expired',
        -- legacy draft statuses (tolerate existing rows)
        'building', 'downloaded'
      ));
...
create index if not exists data_archives_type_created_idx
  on public.data_archives (archive_type, created_at desc);
create index if not exists data_archives_status_idx
  on public.data_archives (status);

comment on table public.data_archives is
  'Compact archive registry. Full ZIP in Storage data-archives. Weekly/monthly NEVER delete production orders.';

alter table public.data_archives enable row level security;
revoke all on table public.data_archives from public, anon, authenticated;
revoke all on sequence public.data_archives_number_seq from public, anon, authenticated;

-- ============================================================
-- 5. Schedules + activity
-- ============================================================

create table if not exists public.data_archive_schedules (
  id uuid primary key default gen_random_uuid(),
  ...
  last_archive_id uuid null references public.data_archives (id) on delete set null,
  metadata jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

alter table public.data_archive_schedules enable row level security;
revoke all on table public.data_archive_schedules from public, anon, authenticated;

insert into public.data_archive_schedules (
  schedule_key, label, cadence, weekday, enabled, next_run_at, metadata
) values (
  'weekly_sunday',
  'Еженедельный архив (воскресенье)',
  'weekly', 0, false,
  (
    (
  ...
  jsonb_build_object('cron_hint', '0 3 * * 0', 'note', 'Wire external cron later')
) on conflict (schedule_key) do nothing;

insert into public.data_archive_schedules (
  schedule_key, label, cadence, day_of_month, enabled, metadata
) values (
  'monthly_first',
  'Ежемесячный архив (1-е число)',
  'monthly', 1, false,
  jsonb_build_object('cron_hint', '0 3 1 * *', 'note', 'Wire external cron later')
) on conflict (schedule_key) do nothing;

create table if not exists public.data_lifecycle_activity (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists data_lifecycle_activity_created_idx
  on public.data_lifecycle_activity (created_at desc);

alter table public.data_lifecycle_activity enable row level security;
revoke all on table public.data_lifecycle_activity from public, anon, authenticated;

-- ============================================================
-- 6. Private Storage bucket: data-archives
-- ============================================================
...
-- No direct browser policies for authenticated: upload/download via server
-- (service_role after verified admin JWT). Deny-by-default for anon/authenticated.
drop policy if exists data_archives_storage_select on storage.objects;
drop policy if exists data_archives_storage_insert on storage.objects;
drop policy if exists data_archives_storage_update on storage.objects;
drop policy if exists data_archives_storage_delete on storage.objects;

-- Immutable: no UPDATE policy. DELETE only via service_role maintenance.
